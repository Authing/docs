...
|mode|Guard 展示模式|[GuardMode](../parameters.md#guardmode)|GuardMode.Normal|
...
|loginMethods|需要使用的普通登录(包括 LDAP)方式列表|[LoginMethods](../parameters.md#loginmethods)[]|Authing 控制台中的配置|
|registerMethods|需要使用的注册方式|[RegisterMethods](../parameters.md#registermethods)[]|Authing 控制台中的配置|
|defaultRegisterMethod|默认展示的注册方式|[RegisterMethods](../parameters.md#registermethods)|Authing 控制台中的配置|
|defaultScenes|打开组件时展示的界面|GuardModuleType|GuardModuleType.LOGIN|
|socialConnections|需要使用的社会化登录列表，**如果在 Authing 控制台中没有配置，则不会显示**|[SocialConnections](../parameters.md#socialconnections)[]|Authing 控制台中的配置|
