...
1. 在项目入口文件中初始化 [AuthenticationClient](/reference-new/web/sdk-for-node/#使用认证模块)。
...
Guard 组件本身基于 [Authing JavaScript SDK](/reference-new/web/sdk-for-node/) 进行封装，当你需要进行一些更高级的操作（如管理用户自定义数据、修改用户资料、退出登录）时：
