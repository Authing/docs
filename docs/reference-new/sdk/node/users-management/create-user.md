...
| 名称 | 位置 | 类型 | 必填 | 默认值 | 描述 |
| ---- | --- | ---- | ---- | ---- | ---- |
| status | body | string |  | Activated | 账户当前状态。 枚举值：`Deleted`,`Suspended`,`Resigned`,`Activated`,`Archived` |
| email | body | string |  |  | 邮箱。 示例值： `test@example.com` |
| passwordEncryptType | body | string |  | none | 加密类型。 枚举值：`sm2`,`rsa`,`none` |
| phone | body | string |  |  | 手机号。 示例值： `176xxxx6754` |
| phoneCountryCode | body | string |  |  | 手机区号。 示例值： `+86` |
| username | body | string |  |  | 用户名，用户池内唯一。 示例值： `bob` |
| name | body | string |  |  | 用户真实名称，不具备唯一性。 示例值： `张三` |
| nickname | body | string |  |  | 昵称。 示例值： `张三` |
| photo | body | string |  |  | 头像链接。 示例值： `https://files.authing.co/authing-console/default-user-avatar.png` |
| gender | body | string |  | U | 性别。 枚举值：`M`,`W`,`U` |
| emailVerified | body | boolean |  |  | 邮箱是否验证。 示例值： `true` |
| phoneVerified | body | boolean |  |  | 手机号是否验证。 示例值： `true` |
| birthdate | body | string |  |  | 出生日期。 示例值： `2022-06-01` |
| country | body | string |  |  | 所在国家。 示例值： `CN` |
| province | body | string |  |  | 所在省份。 示例值： `BJ` |
| city | body | string |  |  | 所在城市。 示例值： `BJ` |
| address | body | string |  |  | 所处地址。 示例值： `北京朝阳` |
| streetAddress | body | string |  |  | 所处街道地址。 示例值： `北京朝阳区 xxx 街道` |
| postalCode | body | string |  |  | 邮政编码号。 示例值： `438100` |
| externalId | body | string |  |  | 第三方外部 ID。 示例值： `10010` |
| departmentIds | body | array |  |  | 用户所属部门 ID 列表。 示例值： `["624d930c3xxxx5c08dd4986e","624d93102xxxx012f33cd2fe"]` |
| customData | body | object |  |  | 自定义数据，传入的对象中的 key 必须先在用户池定义相关自定义字段。 示例值： `[object Object]` |
| password | body | string |  |  | 密码。可选加密方式进行加密，默认为未加密。 示例值： `oqw5bhVmlDwF5qqeVA645bICyMVfFaV3sf3ZTrk5Npcm5dTOmBVo1anyZ5JLfHAz/P45r0QTPo8xS1YdKxIrshx4Ju+g04s9SQqW30ebdVdqcOntIJGAXU6arrkPvfcRFV3ZVTwBdgdRWHMkr5sTcnGNYdgL67P9/jHnzltkLbY=` |
| tenantIds | body | array |  |  | 租户 ID。  |
| identities | body | array |  |  | 第三方身份源（建议调用绑定接口进行绑定）。 示例值： `[object Object]` |
