...
| appType | string | 否 | web | 应用类型。  枚举值：`web`,`spa`,`native`,`api` | `web` |
| defaultProtocol | string | 否 | - | 默认应用协议类型。  枚举值：`oidc`,`oauth`,`saml`,`cas`,`asa` | `oidc` |
...
namespace Example
{
    class Program
    {
      ...
      private static async Task MainAsync()
      {
          ...
          ApplicationPaginatedRespDto  result = await managementClient.CreateApplication
          (  new CreateApplicationDto{                  AppName= "示例应用" ,
                  Template= "github" ,
                  TemplateData= "github" ,
                  ...
                  LogoutRedirectUris= new List<string>{"https://example.com/logout-callback",} ,
          ...
          EnabledBasicLoginMethods= new List<string>{} ,
        ...
        EnabledExtIdpConnIds= new List<ApplicationEnabledExtIdpConnInputDto>
                {
                    new ApplicationEnabledExtIdpConnInputDto
                    {
          ...
          ShowAuthorizationPage= false ,
                ...
                {
                          EnabledBasicRegisterMethods= new List<string>{"EMAIL_CODE","PHONE_CODE",} ,
