...
| 名称 | 类型 | 必填 | 描述 | 示例值 |
| ---- |  ---- | ---- | ---- | ---- |
| status | string | 否 | 账户当前状态。  | 可选枚举值：`Suspended`,`Resigned`,`Activated`,`Archived`,`Deactivated` |
| email | string | 否 | 邮箱，不区分大小写。  |  `test@example.com` |
| passwordEncryptType | string | 否 | 密码加密类型，支持 sm2 和 rsa。  | 可选枚举值：`sm2`,`rsa`,`none` |
...
| gender | string | 否 | 性别。  | 可选枚举值：`M`,`F`,`U` |
...
| departmentIdType | string | 否 | 此次调用中使用的父部门 ID 的类型。  | 可选枚举值：`department_id`,`open_department_id` |
