...
# {{$localeConfig.brandName}} - PHP SDK OIDC 模块
...
- `options` \<array\> 发起授权登录时需要填写的参数。详情请见[使用 OIDC 授权码模式](/federation/oidc/authorization-code/)。
...
AuthenticationClient->validateToken(array $options)

通过 Authing 提供的在线接口验证 Id token 或 Access token。会产生网络请求。

### 参数

- `options.idToken` \<string\> Access token 或 Refresh token，可以从 AuthenticationClient.getAccessTokenByCode 方法的返回值中的 id_token 获得。
...
$res = authenticationClient->validateToken({ idToken: 'idToken 内容' });
```
...
$url = $authenticationClient->buildLogoutUrl([ 'expert' => true, 'idToken' => '待退出用户的 idToken', 'redirectUri' => 'https://authing.cn' ]);
```

## Client Credentials 模式获取 Access Token

AuthenticationClient->getAccessTokenByClientCredentials(string $scope, array $options)
...
- `options.accessKey`，编程访问账号 AccesKesy。
...
- `appId` \<string\> 应用 ID，必填。
- `secret` \<string\> 应用密钥，必填。
- `domain` \<string\> https://{YOUR_DOMAIN}.authing.cn 域名，不含 `https://`，必填。
...
$res = $authenticationClient->getAccessTokenByClientCredentials('email openid profile phone', [ 'accessKey' => '编程访问账号 AK', 'secretKey' => '编程访问账号 SK' ]);
```
