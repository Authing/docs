...
| departmentId | string  | 否 |  | 部门 id，根部门传 `root`。departmentId 和 departmentCode 必传其一。。 示例值： `root` |
| departmentCode | string  | 否 |  | 部门 code。departmentId 和 departmentCode 必传其一。。 示例值： `example` |
