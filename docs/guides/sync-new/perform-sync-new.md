...
<td>3</td>
<td>同步方式</td>
<td>显示创建或修改同步任务时指定的 <b>同步时机</b> 的值。</p>对于手动同步，每次执行，都会生成一条记录。</p>对于定时同步和实时同步，只会生成一条记录，其同步时间以最后一次为准。</p>在 <b>风险策略配置</b> 开启了删除主体自动同步后，执行删除保护操作自动同步并不会影响 <b>同步方式</b> 的值。</td>
</tr>
