...
当你已经构建起用户系统，在某个时刻你的 API 就需要判断当前访问的用户是否能够访问当前资源了。这时候你就需要构建自己的权限系统。权限系统中一个很重要的概念是授权，授权指的是判断用户具备哪些权限的过程，与认证是两个完全不同的含义（有关认证与授权的概念和关联，请参阅 [认证与授权](/concepts/authentication-vs-authorization.md)）。
